<?php
$live_events_no = get_sub_field( 'uat_content_live_auction_events_no' );
...
if(empty($live_events_no)){$live_events_no =10;}
?>
<section class="currant-and-upcoming-auction">
    <div class="container">
        <div class="section-heading">
            <h4><?php the_sub_field( 'uat_content_live_auction_events_title' ); ?></h4>
            <?php $uat_content_live_auction_events_link = get_sub_field( 'uat_content_live_auction_events_link' ); ?>
            <?php if ( $uat_content_live_auction_events_link ) { ?>
            <a href="<?php echo $uat_content_live_auction_events_link['url']; ?>"
                target="<?php echo $uat_content_live_auction_events_link['target']; ?>"><?php echo $uat_content_live_auction_events_link['title']; ?></a>
            <?php } ?>
        </div>
        <?php
		$selected_datetime ="";
		...
		$args_live = array (
					...
					'posts_per_page' =>$live_events_no,
		);
		$args_live['post__in'] = $postids;
		// The Query
		$query = new WP_Query( $args_live );
		if ( $query->have_posts() ) { ?>
		...
		?>
            <div class="owl-item">
				...
				<?php
				  $timer = get_sub_field('uat_content_live_auction_events_timer');
			if($event_status =="uat_live") { 
			global $wpdb;
        ...
        <?php
			...
			echo __( "Live Action Events not found.", 'ultimate-auction-pro-software' );
		} ?>
    </div>
</section>