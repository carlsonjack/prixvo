...
/**
 * Recently Added Auctions Widget
 *
 ...
 *
 */
class UAT_Widget_Latest_Auctions extends WP_Widget
{
	...
	{
		$this->widget_cssclass    = 'woocommerce widget_uat_latest_auctions';
		$this->widget_description = __('Display a list of Recently Added Auctions from your store.', 'ultimate-auction-pro-software');
		$this->widget_id          = 'woocommerce_ua_latest_auctions';
		$this->widget_name        = __('UA Recently Added Auctions', 'ultimate-auction-pro-software');
		/* Widget settings. */
		$uat_widget_arg = array('classname' => $this->widget_cssclass, 'description' => $this->widget_description);
		parent::__construct('uat_latest_auctions', $this->widget_name, $uat_widget_arg);
	...
	{
		wp_cache_delete('widget_uat_latest_auctions', 'widget');
	}
	...
	{
		global $woocommerce;
		$cache = wp_cache_get('widget_uat_latest_auctions', 'widget');
		...
		ob_start();
		extract($args);
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Recently Added Auctions', 'ultimate-auction-pro-software') : $instance['title'], $instance, $this->id_base);
		...
		if ($uat_query->have_posts()) {
			...
			while ($uat_query->have_posts()) {
				...
				$status = json_chk_auction($product_id);
...
?>
								...
								<div class="ua-wid-pro-t">
									...
									<div class="bid-start-with-time">
										...
										<?php } elseif ($status == "future") {  ?>
											<!-- <span class="uat_time_left"><?php _e('Starting Time left', 'ultimate-auction-pro-software'); ?></span> -->
										<?php } ?>
									</div>
								</div>
								<div class="ua-wid-timer">
									<?php if ($uat_hide_time == 1) { ?>
										<?php if (json_chk_auction($product_id) == "live") {
										...
										?>
										<?php } elseif (json_chk_auction($product_id) == "future") {
			...
			<li><?php _e('Recently Added Auctions not found', 'ultimate-auction-pro-software'); ?></li>
		<?php }
		echo '</ul>';
		echo $after_widget;
		wp_reset_postdata();
		...
		echo $content;
		wp_cache_set('widget_uat_latest_auctions', $cache, 'widget');
	}
	...
	function update($new_instance, $old_instance)
	{
		...
		if (isset($alloptions['widget_uat_latest_auctions'])) delete_option('widget_uat_latest_auctions');
		return $instance;
	}
