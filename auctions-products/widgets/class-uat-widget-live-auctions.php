 ...
 *
 */
class UAT_Widget_Live_Auctions extends WP_Widget
{
	...
	{
		$this->widget_cssclass    = 'woocommerce widget_uat_live_auctions';
		$this->widget_description = __('Display a list of Live Auctions from your store.', 'ultimate-auction-pro-software');
		$this->widget_id          = 'woocommerce_ua_live_auctions';
		$this->widget_name        = __('UA Live Auctions', 'ultimate-auction-pro-software');
		/* Widget settings. */
		$uat_widget_arg = array('classname' => $this->widget_cssclass, 'description' => $this->widget_description);
		parent::__construct('uat_live_auctions', $this->widget_name, $uat_widget_arg);
	...
	{
		wp_cache_delete('widget_uat_live_auctions', 'widget');
	}
	...
	{
		global $woocommerce;
		$cache = wp_cache_get('widget_uat_live_auctions', 'widget');
		...
		ob_start();
		extract($args);
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Live Auctions', 'ultimate-auction-pro-software') : $instance['title'], $instance, $this->id_base);
...
?>
			...
			<li><?php _e('Live auction not found', 'ultimate-auction-pro-software'); ?></li>
		<?php }
		echo '</ul>';
		echo $after_widget;
		wp_reset_postdata();
		...
		echo $content;
		wp_cache_set('widget_uat_live_auctions', $cache, 'widget');
	}
	...
	function update($new_instance, $old_instance)
	{
		...
		if (isset($alloptions['widget_uat_live_auctions'])) delete_option('widget_uat_live_auctions');
