...
function jquery_clock_rebind() {
	jQuery(".uwa_auction_product_countdown").each(function (index) {
		...
		var timer_options = {
			...
			format: format,
			compact: compact,
			layout: WooUa.jQuery_timer_layout,
			onExpiry: CheckExpired,
			expiryText: etext
		};
		var countdownString = '';
		if(jQuery(this).hasClass("time_countdown_event")){
			if(days == 0 && minutes == 0 && hours == 0 && seconds == 0){
				location.reload();
			}
			if (uat_data.single_event_page) {
				// event single page
				if (days > 0) {
					timer_options.labels = ['d', 'h', 'm', 's'];
					timer_options.format = 'dHMS';
					timer_options.layout = '{dn}d {hnn}h {mnn}m {snn}s';
				}else if(hours > 0){
					timer_options.labels = ['h', 'm', 's'];
					timer_options.format = 'HMS';
					timer_options.layout = '{hnn}h {mnn}m {snn}s';
				}else if(minutes > 0){
					timer_options.labels = ['m', 's'];
					timer_options.format = 'MS';
					timer_options.layout = '{mnn}m {snn}s';
				}else{
					timer_options.labels = ['s'];
					timer_options.format = 'S';
					timer_options.layout = '{snn}s';
				}
			}else{
				// event list page 
				if (days > 0) {
					countdownString += ' ' + (days > 1 ? uat_data.timer_days : uat_data.timer_day);
					timer_options.labels = ['d'];
					timer_options.format = 'dHMS';
					timer_options.layout = '{dn}' + countdownString;
				}else if(hours > 0){
					timer_options.labels = ['h', 'm', 's'];
					timer_options.format = 'HMS';
					timer_options.layout = '{hnn}h {mnn}m {snn}s';
			...
			if (uat_data.single_product_page) {
				// product single page
				if (days > 0) {
					timer_options.labels = ['d', 'h', 'm', 's'];
					timer_options.format = 'dHMS';
		...
		jQuery(this).WooUacountdown(timer_options);
	});
...
function CheckExpired() {
	...
	request = jQuery.ajax({
		...
		complete: function (data) {
			jQuery('.product-d-timer').hide();
			jQuery('.product-d-timer').remove();
			jQuery('.Current-bid-detail').remove();
			jQuery('.Next-bid-detail').next("small").remove();
			jQuery('.Next-bid-detail').remove();
			jQuery('.bid-status-msg').remove();
			jQuery('.lot-close-date').remove();
			jQuery('.bid-price-selection').remove();
