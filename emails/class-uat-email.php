...
/*
* Ultimate_Auction_Pro_Email class
*
...
if (!class_exists('Ultimate_Auction_Pro_Email')):
    class Ultimate_Auction_Pro_Email
    {
	...
	private $heading = '';
	/**
	 * Ultimate_Auction_Pro_Email constructor.
	 */
	...
	// public function registration_confirm_email( $user_id ) {
	...
	// 				update_user_meta($user_id, 'uat_confirmedcode', $code);
	// 			}
	// 			$to = $this->get_user_email($user_id);
	// 			$subject = $this->replacing_tempalte_tags_string_subject($email_object['subject'],$user_id);
	// 			$message = $this->get_content($email_slug,$user_id);
	...
	// }
	// // Place Bid
	// public function place_bid_email($product_id ,$user_id ) {
    ...
    //         $receipents2 = array();
	// 		if($admin_enable =="yes"){
	// 			$receipents1[] = $this->get_admin_email();
	// 		}
	// 		if($enable =="yes" && !empty($user_id)){
	// 			$receipents2[] = $this->get_user_email($user_id);
	// 		}
	// 			$c = array();
    //             $c = array_merge($receipents1, $receipents2);
    //             $final_receipents = array_unique($c);
	// 			//print_r($final_receipents);
	// 			$total_users = count($final_receipents);
	// 			if ( $total_users > 0 ) {
	// 				$subject = $this->replacing_tempalte_tags_string_subject($email_object['subject'],$user_id);
	// 				$message = $this->get_content($email_slug, $user_id,$product_id );
	// 					foreach ( $final_receipents as $key => $value) {
	// 					 $sent = $this->mail_send( $value, $subject, $message);
	// 					} /* end of foreach */
	// 					return $sent;
	// 			}
	...
	// public function registration_reject_email( $user_id ) {
	// 		$email_slug ="registration_reject";
	// 		$email_object = get_option("uat_email_template_" . $email_slug);
	// 		$enable = $email_object['enable'];
	// 		if($enable =="yes" && !empty($user_id)){
	// 			$to = $this->get_user_email($user_id);
	...
	// public function registration_approved_email($user_id ) {
	// 		$email_slug ="registration_approved";
	// 		$email_object = get_option("uat_email_template_" . $email_slug);
	// 		$enable = $email_object['enable'];
	// 		if($enable =="yes" && !empty($user_id)){
	// 			$to = $this->get_user_email($user_id);
	...
	// //registration_approved notification to admin with link
	// public function registration_approval_admin_email($user_id ) {
	...
	// }
	// Endign Soon
	public function ending_soon_email( $product_id ) {
			 global $wpdb;
			$email_slug ="ending_soon";
			$email_object = get_option("uat_email_template_" . $email_slug);
			$enable = $email_object['enable'];
			if($enable =="yes"){
			$admin_enable = $email_object['admin_enable'];
			$receipents1 = array();
            $receipents2 = array();
			$for_bidders = get_option( 'options_ending_soon_for_bidders', 'on' );
			$for_savelist = get_option( 'options_ending_soon_for_savelist', 'on' );
			...
			if($for_bidders =="on"){
				...
				if (count($ending_auction_users) > 0){
					...
					$receipents1 = $arr_ending_auction_users;
				}
			}
			if($for_savelist =="on"){
				$receipents2 = get_post_meta( $product_id, "uat_auction_saved_id",false);
			}
				$c = array();
                $c = array_merge($receipents1, $receipents2);
                $final_receipents = array_unique($c);
				...
				if ( $total_users > 0 ) {
					...
					foreach ( $final_receipents as $key => $value) {
					$subject = $this->replacing_tempalte_tags_string_subject($email_object['subject'],$value);
					$message = $this->get_content($email_slug, $value ,$product_id );
					$to = $this->get_user_email( $value );
					$sent = $this->mail_send( $to, $subject, $message);
					} /* end of foreach */
					return $sent;
				}
	...
	// /*
	// loose_bid*/
	// public function loser_bid_email( $product_id ,$user_id ) {
	// 		$email_slug ="loose_bid";
	// 		$email_object = get_option("uat_email_template_" . $email_slug);
	// 		$enable = $email_object['enable'];

	// 		$loser_users = $this->get_loser_bidders_list($product_id ,$user_id);
	// 		if($enable =="yes" && !empty($user_id) && count($loser_users) > 0){
	// 			$subject = $this->replacing_tempalte_tags_string_subject($email_object['subject'],$user_id,$product_id);
	// 			$message = $this->get_content($email_slug, $user_id,$product_id );

	// 				foreach ( $loser_users as $key => $value) {
	// 				  $to = $this->get_user_email( $value );
	// 				  $sent = $this->mail_send( $to, $subject, $message);
	// 				} /* end of foreach */
	// 				return $sent;
	...
	// public function get_loser_bidders_list( $product_id ,$winneruser ) {
	// 		global $wpdb;
	// 		$loser_bidders = array();
	// 		$receipents = array();
	// 		$loser_users = $wpdb->get_results("SELECT DISTINCT userid
    //                     FROM ". $wpdb->prefix ."woo_ua_auction_log
    //                     WHERE auction_id = ". $product_id, OBJECT_K); //ARRAY_A

	// 		if (count($loser_users) > 0){
	// 			$arr_loser_users = array_keys($loser_users);
	// 			$loser_bidders = $arr_loser_users;
	// 		}

	// 		if (($key = array_search($winneruser, $loser_bidders)) !== false) {
	// 				unset($loser_bidders[$key]);
	// 			}

	// 	return $loser_bidders;
	...
	/*
	payment_reminder*/
	public function payment_reminder_email($product_id ,$user_id ) {
	...
	// public function payment_confirmation_email($product_id ,$user_id ) {
