...
if (!class_exists('EndingSoonMail')):
    class EndingSoonMail
    {
	...
	public function get_product_auction_endtime( $product_id ) {
		...
		$endtime .=' '. __('Timezone:', 'ultimate-auction-pro-software') . ' <strong>' . wp_timezone_string() . '</strong>';
		return $endtime;
	}
 ...
 // Endign Soon
    public function ending_soon_email( $product_id ) {
		...
		add_post_meta( $product_id, 'uat_auction_ending_soon_sent', $now_timestamp, true );
            $c = array();
            ...
            if ( $total_users > 0 ) {
                ...
                foreach ( $final_receipents as $key => $value) {
							...
							$email_status = $check_email->duplicate_email_check($product_id ,$userid,'ending_soon');
							if( !$email_status )
							{
								$uwt_email_endingsoon_user_enabled = get_user_meta($value,'uwt_email_endingsoon_user_enabled', true);
								if( $uwt_email_endingsoon_user_enabled == "no" )
								{
									continue;
								}
								$subject = $this->replacing_tempalte_tags_string_subject($email_object['subject'],$value,$product_id);
								$message = $this->get_content($email_slug, $value ,$product_id );
								$to = $this->get_user_email( $value );
								$sent = $this->mail_send( $to, $subject, $message);
								$admin_email = get_option( 'admin_email' );
								$check_email->tracking_add($product_id ,$userid,$admin_email,$to,$admin_email?'1':'0','ending_soon',$sent,$subject,$this->get_headers(),$message ,'',!$sent);
							}
                } /* end of foreach */
                // return $sent;
            }
