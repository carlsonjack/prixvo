        ...
        if($enable =="yes" && !empty($user_id) && count($loser_users) > 0){
            $subject = $this->replacing_tempalte_tags_string_subject($email_object['subject'],$user_id,$product_id);
                ...
                foreach ( $loser_users as $key => $value) {
					$email_status = $check_email->duplicate_email_check($auction_id=$product_id ,$user_id=$value,$email_type='loose_bid');
					if( !$email_status )
					{
						$admin_email = get_option( 'admin_email' );
						...
						$check_email->tracking_add($auction_id=$product_id ,$user_id=$value,$user_email=$admin_email,$receiver_email=$this->get_user_email($value),$admin_email=$admin_email?'1':'0',$email_type='loose_bid',$status=$sent,$subject=$subject,$headers=$this->get_headers(),$message=$message ,$amount='',$error=!$sent);
					}
                } /* end of foreach */
                // return $sent;
        }
