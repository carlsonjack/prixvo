...
if (!class_exists('PaymentConfirmationMail')):
    class PaymentConfirmationMail
    {
	...
	private $heading = '';
	/**
	 * PaymentConfirmationMail constructor.
	 */
	...
	public function get_content( $email_slug, $order_id,$user_id) {
			ob_start();
			...
			ob_end_clean();
			return $this->replacing_tempalte_tags_string_body($email_content,$user_id,$order_id);

	}


	public function replacing_tempalte_tags_string_body( $string ,$user_id,$order_id="") {
	...
	public function replacing_tempalte_tags_string_subject( $string , $order_id, $user_id  ) {
	...
	public function payment_confirmation_email($order_id) {
        ...
        if($enable =="yes" && !empty($user_id)){
			...
			if( $uwt_email_payment_confirmation_user_enabled == "no" )
			{
				return false;
			}

		   $to = $this->get_user_email($user_id);
            $subject = $this->replacing_tempalte_tags_string_subject($email_object['subject'],$order_id,$user_id );
            $message = $this->get_content($email_slug, $order_id,$user_id );
	...
	$orderHTML.='<thead>';
		...
		foreach ( $order_items as $item_id => $item ) {
				...
				$orderHTML.='<td class="woocommerce-table__product-name product-name">';
					...
					if ( $refunded_qty ) {
					  ...
					  $woocommerce_order_item_quantity_html=apply_filters( 'woocommerce_order_item_quantity_html', ' <strong class="product-quantity">' . sprintf( '&times;&nbsp;%s', $qty_display ) . '</strong>', $item ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		$orderHTML.='<tfoot>';
			...
			foreach ( $order->get_order_item_totals() as $key => $total ) {
				 ...
				 $payment_method=( 'payment_method' === $key ) ? __( $total['value'] ) : wp_kses_post( $total['value'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
