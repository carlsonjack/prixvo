...
if (!class_exists('PaymentHoldMail')):
    class PaymentHoldMail
    {
	...
	public function get_content( $email_slug, $user_id="", $product_id="", $hold_amount = 0) {
			ob_start();
			...
			ob_end_clean();
			return $this->replacing_tempalte_tags_string_body($email_content,$user_id, $product_id, $hold_amount);
	}
	public function replacing_tempalte_tags_string_body( $string ,$user_id="", $product_id="", $hold_amount = 0 ) {
		...
		$string = str_replace( '{hold_amount}', $hold_amount, $string );
 ...
 // Endign Soon
    public function payment_hold_email( $product_id, $userid, $hold_amount = 0 ) {
        global $wpdb;
		...
		if($enable =="yes")
		{
			...
			$subject = $this->replacing_tempalte_tags_string_subject($email_object['subject'],$userid, $product_id);
			$message = $this->get_content($email_slug, $userid ,$product_id,$hold_amount );
			$admin_email = get_option( 'admin_email' );
			$to = $admin_email;
			$sent = $this->mail_send( $to, $subject, $message);
			$check_email->tracking_add($product_id ,$userid,$admin_email,$to,$admin_email?'1':'0','payment_hold',$sent,$subject,$this->get_headers(),$message ,'',!$sent);
		}
        return false;
    }
