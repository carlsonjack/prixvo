	...
	public function place_bid_email($product_id ,$user_id ) {
			...
			if($enable =="yes" && !empty($user_id)){
				if($admin_enable =="yes"){
					...
					$check_email->tracking_add($auction_id=$product_id ,$user_id=$user_id,$user_email=$admin_email,$receiver_email=$admin_email,$admin_email=$admin_email?'1':'0',$email_type='place_bid',$status=$sent,$subject=$subject,$headers=$this->get_headers(),$message=$message ,$amount=$this->get_product_bid_value( $product_id ),$error=!$sent);
			...
			if($enable =="yes" && !empty($user_id)){
				...
				if( $uwt_email_placebid_user_enabled == "no" )
				{
					return false;
				}
				$admin_email = get_option( 'admin_email' );
				...
				$sent = $this->mail_send( $user_email, $subject, $message);
				$check_email->tracking_add($auction_id=$product_id ,$user_id=$user_id,$admin_email,$receiver_email=$user_email,$admin_email=$admin_email?'1':'0',$email_type='place_bid',$status=$sent,$subject=$subject,$headers=$this->get_headers(),$message=$message ,$amount=$this->get_product_bid_value( $product_id ),$error=!$sent);
			}
