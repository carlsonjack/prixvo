...
if (!class_exists('AnswerMail')):
	...
	private $heading = '';
	/**
	 * QuestionAnswerMail constructor.
	 */
	...
	public function get_content( $email_slug, $user_id="", $product_id="", $user_type='',$asked_by_id="") {
			ob_start();
			...
			ob_end_clean();
			return $this->replacing_tempalte_tags_string_body($email_content,$user_id, $product_id,$asked_by_id);
	}
	public function replacing_tempalte_tags_string_body( $string ,$user_id="", $product_id="",$asked_by_id="" ) {
		$username =	$this->get_username( $asked_by_id );
		$seller_name =	$this->get_username( $user_id );
	...
	public function replacing_tempalte_tags_string_subject( $string , $user_id="" , $product_id="",$asked_by_id=""  ) {
		$username =	$this->get_username( $asked_by_id );
		$seller_name =	$this->get_username( $user_id );
		$product_name =	$this->get_product_name( $product_id );
		$string = str_replace( '{username}', $username, $string );
    ...
    /*
	won_bid*/
	public function answer_email($answers_id) {
		...
		$ans_re = $wpdb->get_results("SELECT * FROM ".$ua_auction_answer." where answers_id=".$answers_id);
		$answered_by_id=$ans_re[0]->answered_by_id;
		$user_id=$answered_by_id;
		$qa_re = $wpdb->get_results("SELECT * FROM ".$ua_auction_question." where question_id=".$ans_re[0]->question_id);
		$post_owner_id=$qa_re[0]->post_owner_id;
		$asked_by_id=$qa_re[0]->asked_by_id;
		$user_id=$post_owner_id;
		$product_id=$qa_re[0]->post_id;
		$user_info = get_userdata($answered_by_id);
		$to = $user_info->display_name;
        $email_slug ="answer";
        $email_object = get_option("uat_email_template_" . $email_slug);
		$to = $this->get_user_email($user_id);
		$subject = $this->replacing_tempalte_tags_string_subject($email_object['subject'],$user_id,$product_id,$asked_by_id );
		$message = $this->get_content($email_slug, $user_id,$product_id , $user_type='',$asked_by_id);
