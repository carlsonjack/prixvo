...
/*
* RegistrationApprovalAdminMail class
*
...
if (!class_exists('RegistrationApprovalAdminMail')):
    class RegistrationApprovalAdminMail
    {
	...
	private $heading = '';
	/**
	 * RegistrationApprovalAdminMail constructor.
	 */
    ...
    public function registration_approval_admin_email($user_id ) {
        ...
        if($enable =="yes" && !empty($user_id)){
			...
			$admin_email = get_option( 'admin_email' );
			$check_email->tracking_add($auction_id='0' ,$user_id=$user_id,$user_email=$admin_email,$receiver_email=$to,$admin_email=$admin_email?'1':'0',$email_type='registration_approved_admin',$status=$sent,$subject=$subject,$headers=$this->get_headers(),$message=$message ,$amount='',$error=!$sent);
