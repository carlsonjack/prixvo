...
if (!class_exists('RegistrationConfirmMail')):
    class RegistrationConfirmMail
    {
	...
	private $heading = '';
	/**
	 * RegistrationConfirmMail constructor.
	 */
	...
	public function replacing_tempalte_tags_string_body( $string ,$user_id="", $product_id="" ) {
		...
		$first_name =	$this->get_first_name( $user_id );
    ...
    public function registration_confirm_email( $user_id ) {
        ...
        if($enable =="yes" && !empty($user_id)){
			...
			$admin_email = get_option( 'admin_email' );
			$check_email->tracking_add($auction_id='0' ,$user_id=$user_id,$user_email=$admin_email,$receiver_email=$to,$admin_email=$admin_email?'1':'0',$email_type='registration_confirm',$status=$sent,$subject=$subject,$headers=$this->get_headers(),$message=$message ,$amount='',$error=!$sent);
