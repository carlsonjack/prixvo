...
if (!class_exists('SaveSearchMail')):
    class SaveSearchMail
    {
    ...
    /*
	email_save_search*/
	public function save_search_bid_email($product_id ,$user_id ) {
        ...
        $admin_enable = $email_object['admin_enable'];
			...
			$check_email = new EmailTracking();
			  $email_status = $check_email->duplicate_email_check($product_id ,$user_id,'email_save_search');
			 if( !$email_status )
			{ 
				...
				$admin_email = get_option( 'admin_email' );
				$check_email->tracking_add($auction_id=$product_id ,$user_id=$user_id,$user_email=$admin_email,$receiver_email=$to,$admin_email=$admin_email?'1':'0',$email_type='email_save_search',$status=$sent,$subject=$subject,$headers=$this->get_headers(),$message=$message ,$amount='',$error=!$sent);
