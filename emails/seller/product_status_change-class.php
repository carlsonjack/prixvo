...
if (!class_exists('ProductStatusChange')):
	...
	public function replacing_tempalte_tags_string_body( $string ,$user_id="", $product_id="" ) {
		$product_seller_id = get_post_meta($product_id, 'uat_seller_id', true);
		$seller_username =	$this->get_username( $product_seller_id );
		$seller_email =	$this->get_user_email( $product_seller_id );
		$string = str_replace( '{seller_name}', $seller_username, $string );
    ...
    /*
	loose_bid*/
	public function status_change_email($email_slug, $product_id ) {
        $email_object = get_option("uat_email_template_" . $email_slug);
		$product_seller_id = get_post_meta($product_id, 'uat_seller_id', true);
		if(empty($product_seller_id)){
			return false;
		}
        $enable = $email_object['enable'];
        if($enable =="yes" && !empty($product_seller_id)){
            $subject = $this->replacing_tempalte_tags_string_subject($email_object['subject'],$product_seller_id,$product_id);
			$message = $this->get_content($email_slug, $product_seller_id,$product_id );
			$to = $this->get_user_email( $product_seller_id );
			$check_email = new EmailTracking();
			$uwt_email_seller_enabled = get_user_meta($product_seller_id,$email_slug.'_enabled', true);
			if( $uwt_email_seller_enabled == "yes" )
			{
				$email_status = $check_email->duplicate_email_check($auction_id=$product_id ,$user_id=$product_seller_id,$email_type=$email_slug);
				if( !$email_status )
				{
					$sent = $this->mail_send( $to, $subject, $message);
					$admin_email = get_option( 'admin_email' );
					$check_email->tracking_add($auction_id=$product_id ,$user_id=$product_seller_id,$user_email=$admin_email,$receiver_email=$to,$admin_email=$admin_email?'1':'0',$email_type=$email_slug,$status=$sent,$subject=$subject,$headers=$this->get_headers(),$message=$message ,$amount='',$error=!$sent);
				}
			}
