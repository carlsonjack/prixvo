 ...
 *
 */
class UAT_Widget_Ending_Soon_Events extends WP_Widget
{
	...
	{
		$this->widget_cssclass    = 'woocommerce widget_uat_ending_soon_events';
		$this->widget_description = __('Display a list of Ending Soon Events from your store.', 'ultimate-auction-pro-software');
		$this->widget_id          = 'woocommerce_ua_ending_soon_events';
		$this->widget_name        = __('UA Ending Soon Events', 'ultimate-auction-pro-software');
		/* Widget settings. */
		$uat_widget_arg = array('classname' => $this->widget_cssclass, 'description' => $this->widget_description);
		parent::__construct('uat_ending_soon_events', $this->widget_name, $uat_widget_arg);
	...
	{
		wp_cache_delete('widget_uat_ending_soon_events', 'widget');
	}
	...
	{
		global $woocommerce;
		$cache = wp_cache_get('widget_uat_ending_soon_events', 'widget');
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Ending Soon Events', 'ultimate-auction-pro-software') : $instance['title'], $instance, $this->id_base);
		if (!$number = (int) $instance['number'])
			...
			$number = 10;
		$postin_array = array();
		$postids = uat_get_ending_soon_events_ids($instance['days_left']);
...
?>
			...
			<li><?php _e('Ending Soon events not found', 'ultimate-auction-pro-software'); ?></li>
		<?php }
		echo '</ul>';
		echo $after_widget;
		wp_reset_postdata();
		...
		echo $content;
		wp_cache_set('widget_uat_ending_soon_events', $cache, 'widget');
	}
	...
	function update($new_instance, $old_instance)
	{
		...
		if (isset($alloptions['widget_uat_ending_soon_events'])) delete_option('widget_uat_ending_soon_events');
