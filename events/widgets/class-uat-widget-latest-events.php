 ...
 *
 */
class UAT_Widget_Latest_Events extends WP_Widget
{
	...
	{
		$this->widget_cssclass    = 'woocommerce widget_uat_latest_events';
		$this->widget_description = __('Display a list of Recently Added Auction Events from your store.', 'ultimate-auction-pro-software');
		$this->widget_id          = 'woocommerce_ua_latest_events';
		$this->widget_name        = __('UA Recently Added Auction Events', 'ultimate-auction-pro-software');
		/* Widget settings. */
		$uat_widget_arg = array('classname' => $this->widget_cssclass, 'description' => $this->widget_description);
		parent::__construct('uat_latest_events', $this->widget_name, $uat_widget_arg);
	...
	{
		wp_cache_delete('widget_uat_latest_events', 'widget');
	}
	...
	function widget($args, $instance)
	{
		...
		$cache = wp_cache_get('widget_uat_latest_events', 'widget');
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Recently Added Auction Events', 'ultimate-auction-pro-software') : $instance['title'], $instance, $this->id_base);
		...
		$postids  = uat_get_all_events_ids();
		if (empty($postids)) {
			$postids[] = array();
		}
...
?>
						...
						<a href="<?php echo get_permalink(); ?>">
							...
							<div class="ua-wid-discription">
								...
								<div class="ua-wid-timer">
									...
									<?php if ($uat_hide_time == 1 && $event_status != "uat_past") {
			...
			<li><?php _e('Recently Added Auction events not found', 'ultimate-auction-pro-software'); ?></li>
		<?php }
		echo '</ul>';
		echo $after_widget;
		wp_reset_postdata();
		...
		echo $content;
		wp_cache_set('widget_uat_latest_events', $cache, 'widget');
	}
	...
	function update($new_instance, $old_instance)
	{
		...
		if (isset($alloptions['widget_uat_latest_events'])) delete_option('widget_uat_latest_events');
