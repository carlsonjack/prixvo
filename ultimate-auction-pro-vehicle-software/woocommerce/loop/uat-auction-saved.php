...
<div class="like-product loop   <?php echo $hide_desktop_class;?> <?php echo $hide_mobile_class;?>">
	...
	<a href="javascript:void(0)"  data-auction-id="<?php echo esc_attr( $product->get_id() ); ?>"	class="remove-uat uat-savedlist-action-loop" title="<?php _e('Click to save remove auction product', 'ultimate-auction-pro-software'); ?>">
	...
	<?php 
		...
		echo '<span class="like-count">'.getProductWatchingCount($product_id).'</span>';
	?>
</div>
...
<?php } else { ?>
	...
	<?php 
		$previous_count = get_post_meta( $product->get_id(), 'uat_auction_saved_count', true);
		$previous_count = $previous_count > 0 ? $previous_count  : 0;
		echo '<span class="like-count">'.$previous_count.'</span>';
	?>
	</div>
