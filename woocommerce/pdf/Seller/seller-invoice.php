...
<h1 class="document-type-label">
	<?php 
	if(!empty($seller_id)){
		echo __( 'Seller receipt', 'ultimate-auction-pro-software' );
	}else{
			...
			if($has_seller && !empty($seller_id)){
                try {
                    ...
                    if ( $uat_seller_id != $seller_id) {
						...
						// print_r($item);
						// echo "</pre>";
						$subtotal = $subtotal + $item['item']['subtotal'];
						$tax_total = $tax_total + $item['item']['total_tax'];
					}
                } catch (\Throwable $th) {
                    //throw $th;
                }
					...
					<tfoot>
						<?php 
							if($has_seller && !empty($seller_id)){
								?>
									<tr>
										<th class="description"><?php echo __('Subtotal', 'ultimate-auction-pro-software'); ?></th>
										<td class="price"><span class="totals-price"><?php echo wc_price($subtotal); ?></span></td>
									</tr>	
								<?php
							}else{
