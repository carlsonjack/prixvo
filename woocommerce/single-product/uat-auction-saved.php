...
<div class="like-product  <?php echo $hide_desktop_class;?> <?php echo $hide_mobile_class;?>">
	...
	<a href="javascript:void(0)" data-page-name="sing" data-auction-id="<?php echo esc_attr( $product_id ); ?>" 
		class="remove-uat uat-savedlist-action" title="<?php _e('Click to save remove auction product', 'ultimate-auction-pro-software'); ?>">
		<i class="fas fa-heart"></i>
		<span><?php echo getProductWatchingCount($product_id); ?></span>
	</a>
	<?php else : ?>			
		<a href="javascript:void(0)" data-page-name="sing" data-auction-id="<?php echo esc_attr( $product_id ); ?>" 
		class="add-uat uat-savedlist-action" <?php if($user_id == 0) echo "no-action ";?>  title="<?php if($user_id == 0) echo 'Please sign in to add auction to save.';?>">
...
<?php } else { ?>
	...
	<a href="<?php echo esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))); ?>" >
		<i class="far fa-heart"></i><span><?php echo getProductWatchingCount($product_id); ?></span>		
	</a>
	<?php } else { ?>
